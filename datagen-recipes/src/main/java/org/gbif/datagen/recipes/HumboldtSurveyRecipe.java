 ...
 *
 *  <p>survey-target / survey-survey-target: {@code Dist.uniform(0, 3)} on the junction table
 *  means some surveys get zero survey-target rows and some get 1-3, covering both of
 *  {@code HumboldtExtensionBuilder}'s paths (single Humboldt row per event, and fan-out to one
 *  row per survey-target).
 *
 *  <pre>
 ...
 *  occurrence (~30,000-45,000) ┘               ▼                    │
 *   sighting/absence              survey-survey-target ────────────┘
 *   records                       (0-3 per survey)
 *          │
 *          │ organism_fk (25%)
 *          ▼
 *     organism (4,000)
...
public final class HumboldtSurveyRecipe {
  ...
  public static DataPackageSpec build(SchemaBundle bundle) {
    return Blueprint.forSchema(bundle)
      ...
      .allFields()
      .rows(2_500)
      // eventID: weakPk, not req+unique. Explicit generator rather than the schema default.
      ...
      // geospatialScopeAreaValue, totalAreaSampledValue, eventDurationValue, and
      // samplingEffortValue all carry a schema min:0, so the schema default always gives them
      // a real number. Their paired unit fields have no such constraint, so pinning all four
      // units explicitly here to keep each value/unit pair non-null together.
      .field("geospatialScopeAreaUnit", Gen.constant("m2"))
      .field("totalAreaSampledUnit", Gen.constant("m2"))
      .field("eventDurationUnit", Gen.constant("hours"))
      .field("samplingEffortUnit", Gen.constant("person-hours"))

      // Small reused pool of standardized target definitions.
      ...
      // Dist.uniform(0, 3): some surveys get no linked target, some get 1-3.
      .resource("survey-survey-target")
      .allFields()
      .per("survey", Dist.uniform(0, 3))
      // surveyTarget_fk: schema-fk default, a lookup into the survey-target pool with
      // replacement.
      ...
      .resource("occurrence")
      .allFields()
      // Changed from Dist.zipf(1.3).max(25) to Dist.uniform(1, 25): every event now gets at
      // least one occurrence, up to 25.
      .per("event", Dist.uniform(1, 25))
      .field("scientificName", DwcGen.scientificName())
      // occurrenceID: weakPk, not req+unique. Explicit generator rather than the schema default.
